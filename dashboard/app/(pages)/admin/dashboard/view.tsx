...
export default function DashboardView({
  translations,
  charts,
}: Readonly<{ translations: Record<string, any>; charts: Record<string, any> }>) {
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);

  const [position, setPosition] = useState("right") as [String, Function];

  const [useCharts, setUseCharts] = useState(charts) as [Record<string, any>, Function];

  const updateChartsPosition = useCallback(() => {
    if (
      !Object.keys(useCharts).length ||
      (from("tablet") && position !== "bottom") ||
      (!from("tablet") && position !== "right")
    ) {
      const newPosition = from("tablet") ? "bottom" : "right";
      setPosition(newPosition);

      const changedCharts = Object.keys(charts).reduce((acc: any, key: string) => {
        acc[key] = charts[key];
        if (acc[key]?.options?.legend) {
