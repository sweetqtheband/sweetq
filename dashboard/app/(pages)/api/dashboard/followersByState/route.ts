...
export async function GET(req: NextRequest) {
  ...
  const data = followers
    .filter((follower: Follower) => !follower?.unfollow)
    .reduce((acc: Record<string, any>, follower: Follower) => {
  ...
  const countries = Object.keys(data).filter((key) => !["total", "unknown"].includes(key));
  ...
  data.translations = await Promise.all(
    countries.map(async (id) => {
      ...
      const states = await Promise.all(
        Object.keys(data[id]).map(async (stateId: String) => {
