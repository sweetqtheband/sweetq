...
const updateDataFromSpotify = async (artistId, trackId) => {
  ...
  const totalListeners = /(\d+)/g.exec((await page.evaluate(el => el.textContent, text)))[0];

  const href = await page.waitForSelector(`[href$="/${trackId}"]`);
  const plays = await page.evaluateHandle(el => el.parentElement.parentElement.nextElementSibling, href);
